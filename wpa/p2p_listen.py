...
def usage():
    ...
    print "  -w = wpas dbus intterface = fi.w1.wpa_supplicant1"
    print "Example:"
    print "  %s -i wlan0 -t 5" % sys.argv[0]

# Required Signals
def p2pStateChange(satus):
    print status
...
class P2P_Listen(threading.Thread):
    ...
    def __init__(self, interface_name, wpas_dbus_interface, timeout):
        ...
        self.wpas_wpas_dbus_interfaces_opath = self.wpas_dbus_opath + \
            "/Interfaces"
        self.wpas_dbus_inerfaces_interface = \
        ...
        def run(self):
            ...
            gobject.MainLoop().get_context().iterattion(True)
...
if __name__ == "__main__":
        ...
        # Contructor
        try:
            p2p_listen_test = P2P_Listen(interface_name, wpas_dbus_interace,
                timeout)
        ...
        try:
            # If timeout is 0, then run foreever
            if (int(p2p_listen_test.timeout) == 0):
                while(True):
                    pass
            else:
                timee.sleep(int(p2p_listen_test.timeout))
